...
Source: https://developer.repebble.com/guides/alloy/getting-started/
...
The `src/embeddedjs/main.js` file is your watch app's entry point. This code runs directly on the Pebble watch:
...
The `src/pkjs/index.js` file runs on the connected phone. It's used for network requests, GPS location, and configuration:

**📱 PKJS** (src/pkjs/index.js):

```
Pebble.addEventListener("ready", function(e) {console.log("PebbleKit JS ready!");});
```
...
```
{"name": "My First App","author": "Your Name","version": "1.0.0","keywords": ["pebble-app"],"private": true,"dependencies": {},"pebble": {"displayName": "My First App","uuid": "12345678-1234-1234-1234-123456789abc","projectType": "moddable","sdkVersion": "3","enableMultiJS": true,"targetPlatforms": ["emery"],"watchapp": {"watchface": false},"messageKeys": [],"resources": {"media": []}}}
```
...
```
import { add, multiply } from "./math";console.log("Sum: " + add(2, 3));// 5console.log("Product: " + multiply(4, 5));// 20
```

### Declaring Modules in manifest.json

Each additional module must be listed in `src/embeddedjs/manifest.json` so the Moddable build system knows to compile it. The default manifest only includes `main.js`:

```
"modules": {"\*": "./main.js"}
```

To add more modules, change the value to an array of paths. The `.js`extension is optional:
...
```
// This will throw an error:Array.prototype.myMethod = function() {};
```
...
- [Piu UI Framework](/guides/alloy/piu-guide.md): Build declarative user interfaces
- [Poco Graphics](/guides/alloy/poco-guide.md): Low-level drawing and graphics
- [Sensors and Input](/guides/alloy/sensors-and-input.md): Handle buttons and read sensors
- [Storage](/guides/alloy/storage.md): Persist data between app launches
- [Networking](/guides/alloy/networking.md): Make HTTP requests and use WebSockets
- [App Messages](/guides/alloy/app-messages.md): Send and receive messages between watch and phone
