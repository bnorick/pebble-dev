...
Source: https://developer.repebble.com/community/submission/

We are putting together a collection of community resources for a new section on the Pebble Developer Website, and we need _your_ contributions!
...
To add your resource to the collection, simply [fork this GitHub repository](https://github.com/pebble/community-resources/fork), follow the relevant guide below to create a Markdown file in the right folder, and submit a pull request.
...
To add your app, create a new Markdown file in the correct folder with the same name as your app (in a URL friendly style, such as `multi-timer.md` or`uk-transport.md`). Examples:
...
```
---name: Multi Timercreator: Matthew Tolelicense: mitlink: https://github.com/smallstoneapps/multi-timer/appstore: 52bda40bdc2f4f552e000089tags:- watchapp- js- storage---
```
...
These are all the possible tags for your app. If you think a tag should be added to the list, get in touch with us [using the contact form](/contact.md).
...
To add your tool, create a new Markdown file in the correct folder with the same name as your tool, in a URL-friendly style, such as `pebble-local-simulator.md`or `watchface-generator.md`. Examples:

```
tools/pebble-local-simulator.mdtools/watchface-generator.md
```

At the top of your Markdown file, you will need to have a metadata block that contains the following properties.

```
---name: Watchface Generatorcreator: Paul Rodelink: http://watchface-generator.de---
```
...
To add your library create a new Markdown file in the `libraries` folder with the same name as your library, in a URL-friendly style, such as`js-message-queue.md` or `clock-layer.md`. Examples:
...
```
---name: JS Message Queuecreator: Matthew Tolelicense: mitlink: https://github.com/smallstoneapps/js-message-queue/language: js---
```
